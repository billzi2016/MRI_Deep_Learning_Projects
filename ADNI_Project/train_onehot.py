...
# ── Config ──────────────────────────────────────────────────────────────
CSV_PATH    = 'data.csv'          # columns: nii.gz_name, stage
NII_DIR     = '../ADNI'           # directory containing .nii.gz files
SAVE_PATH   = 'best_model_adni.pt'
...
def train():
    ...
    # map any bare 'MCI' → 'LMCI' for backward compatibility
    df['stage'] = df['stage'].replace({'MCI': 'LMCI'})

    train_df, val_df = train_test_split(
        df, test_size=0.2, random_state=42, stratify=df['stage']
    )

    train_loader = DataLoader(
        ADNIDataset(train_df, NII_DIR),
        batch_size=BATCH_SIZE, shuffle=True,
        num_workers=NUM_WORKERS, pin_memory=True
    )
    val_loader = DataLoader(
        ADNIDataset(val_df, NII_DIR),
    ...
    # class-frequency weighted loss to handle imbalance across 5 stages
    counts    = df['stage'].map(LABEL_MAP).value_counts().sort_index()
