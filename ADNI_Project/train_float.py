...
# ── Config ──────────────────────────────────────────────────────────────
CSV_PATH    = 'data.csv'          # columns: nii.gz_name, stage, score
NII_DIR     = '../ADNI'
SAVE_PATH   = 'best_model_adni_float.pt'
...
def train():
    ...
    train_df, val_df = train_test_split(df, test_size=0.2, random_state=42, stratify=df['stage'])
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        with torch.no_grad():
            for imgs, scores in val_loader:
                imgs, scores = imgs.to(device), scores.to(device)
                with autocast():
                    val_loss += criterion(model(imgs), scores).item()
